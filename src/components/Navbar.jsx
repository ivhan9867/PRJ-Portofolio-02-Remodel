...
export default function Navbar() {
  ...
  // On case study page, anchor links go back to home
  ...
  return (
    ...
    >
      <Link to="/" className="font-display text-[1rem] text-[#e8e6f0] hover:text-accent transition-colors duration-300">
      ...
      <AnimatePresence>
        {open && (
          ...
          >
            <Link to="/" onClick={()=>setOpen(false)} className="font-display text-xl text-[#e8e6f0]">Home</Link>
            {[['Work','work'],['Experience','experience']].map(([l,h])=>(
              <a key={l} href={`#${h}`} onClick={()=>setOpen(false)} className="font-display text-xl text-[#e8e6f0]">{l}</a>
            ))}
